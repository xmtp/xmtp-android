...
class InstrumentedTest {
    ...
    fun testPublishingAndFetchingContactBundlesWithWhileGeneratingKeys() {
        val aliceWallet = PrivateKey.newBuilder().build().generate()
        val alice = PrivateKeyOuterClass.PrivateKeyBundleV1.generate(wallet = aliceWallet)
        ...
        val noContactYet = client.getUserContact(peerAddress = aliceWallet.walletAddress)
        assertNull(noContactYet)
        client.publishUserContact()
        val contact = client.getUserContact(peerAddress = aliceWallet.walletAddress)
