...
class ConversationsTest {
    ...
    fun testsCanSyncAllConversationsFiltered() {
        ...
        val group =
            runBlocking { boClient.conversations.newGroup(listOf(caro.walletAddress)) }
        assertEquals(runBlocking { boClient.conversations.syncAllConversations() }, 2)
        assertEquals(
            runBlocking { boClient.conversations.syncAllConversations(consentState = ConsentState.ALLOWED) },
            2
        )
        runBlocking { group.updateConsentState(ConsentState.DENIED) }
        assertEquals(
        ...
        assertEquals(
            runBlocking { boClient.conversations.syncAllConversations(consentState = ConsentState.DENIED) },
            1
        )
        assertEquals(runBlocking { boClient.conversations.syncAllConversations() }, 2)
    }
