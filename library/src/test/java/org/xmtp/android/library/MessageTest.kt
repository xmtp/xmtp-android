...
class MessageTest {
    ...
    fun testCanDecrypt() {
        ...
        val ciphertext = CipherText.newBuilder().apply {
            aes256GcmHkdfSha256.toBuilder().gcmNonce = nonce.toByteString()
            aes256GcmHkdfSha256.toBuilder().hkdfSalt = salt.toByteString()
            aes256GcmHkdfSha256.toBuilder().payload = payload.toByteString()
        }.build()
    ...
    fun testCanReadGzipCompressedMessages() {
        ...
        val key = PrivateKeyOuterClass.PrivateKey.newBuilder().also {
            it.secp256K1.toBuilder().bytes = keyBytes.toByteString()
            it.publicKey.toBuilder().secp256K1Uncompressed.toBuilder().bytes =
